...
const toCapital = str => str.charAt(0).toUpperCase() + str.slice(1); // prima litera + restu stringului
...
export const includeUpdatableResource = (Component, resourceUrl, resourceName) =>{
    ...
    return props=>{ 
        ...
        const onPost = async () =>{
            const response = await axios.post(resourceUrl,{[resourceName] : resourceName})
            setInitialResource(response.data)
            setResource(response.data)
        }
